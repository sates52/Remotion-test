...
const { pickGrammar, loadGrammarHistory, publishedSlugs } = require("./lib/thumbnail-grammar");
...
const pick = pickGrammar({ slug: SLUG, engine, history });
...
console.log(`  → pick: ${pick.layout} / ${pick.textPos} / ${pick.type} / ${pick.treatment} / ${pick.accent}  (score ${pick._score})`);
...
const { _score, ...grammar } = pick;
thumb.grammar = { ...grammar, ...(thumb.grammar && thumb.grammar.sceneId ? { sceneId: thumb.grammar.sceneId } : {}) };
