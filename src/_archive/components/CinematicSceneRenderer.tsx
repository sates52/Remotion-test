...
import { GildedOverlay, VignettePulsar, HauntedArrow } from './effects';
...
const CinematicAnimatedAsset: React.FC<{
    ...
    sceneOpacity = 1.0, sceneBlendMode = 'normal' 
}) => {
    ...
    switch (animation.type) {
        ...
        case 'arcaneSwirl': {
            const swirlRotate = interpolate(sceneProgress, [0, 1], [0, 20]);
            const swirlScale = interpolate(sceneProgress, [0, 1], [1.1, 1.5]);
            const swirlBlur = interpolate(sceneProgress, [0, 0.1, 0.9, 1], [4, 0, 0, 2]);
            transform = `rotate(${swirlRotate}deg) scale(${swirlScale})`;
            filter = swirlBlur > 0.1 ? `blur(${swirlBlur}px)` : '';
            break;
        }

        case 'kenburns_diagonal': {
            const scale = interpolate(sceneProgress, [0, 1], [1, 1.25]);
            const x = interpolate(sceneProgress, [0, 1], [-5, 5]);
            const y = interpolate(sceneProgress, [0, 1], [3, -3]);
            transform = `scale(${scale}) translate(${x}%, ${y}%)`;
            break;
        }

        case 'tilt_pan_zoom': {
            const scale = interpolate(sceneProgress, [0, 0.5, 1], [1.1, 1.3, 1.15]);
            const rotate = interpolate(sceneProgress, [0, 1], [-2, 2]);
            const x = interpolate(sceneProgress, [0, 1], [-10, 10]);
            transform = `scale(${scale}) rotate(${rotate}deg) translateX(${x}%)`;
            break;
        }

        case 'focus_drift': {
            const scale = 1.15;
            const x = Math.sin(sceneProgress * Math.PI) * 8;
            const blur = interpolate(sceneProgress, [0, 0.2, 0.8, 1], [4, 0, 0, 4]);
    ...
    return (
        <div
            style={{
                ...
                filter,
                opacity: (animatedOpacity ?? 1) * sceneOpacity,
                mixBlendMode: sceneBlendMode as any,
...
export const CinematicSceneRenderer: React.FC<CinematicSceneRendererProps> = ({
    ...
    const sceneDuration = (scene.endTime - scene.startTime) || 10;
    const sceneProgress = Math.max(0, Math.min(0.999, frame / (sceneDuration * fps)));

    // Color grading shift throughout scene
    const colorGradeHue = interpolate(sceneProgress, [0, 1], [0, 8]);
    ...
    return (
        ...
        >
            ...
            >
                {/* Chromatic Aberration Layer (Fever Dream) */}
                {effectIntensity > 0.6 && (
                    <div style={{
                        position: 'absolute',
                        inset: 4,
                        ...
                        transform: `scale(1.02) translate(${Math.sin(frame / 40) * 2}px, ${Math.cos(frame / 60) * 2}px)`,
                        pointerEvents: 'none',
                        zIndex: -1,
                    }}>
                        {/* Recursive render or simple clone would go here, but for now just a shift */}
                    </div>
                )}
                {/* Render scene assets with fallbacks for flat JSON structures */}
                {(scene.assets || [{ path: (scene as any).image, type: 'image' } as SceneAsset]).map((asset, idx) => {
                    const animations = scene.animations || [];
                    const animation = animations[idx] || animations[0] || {
                        type: 'fade',
                        easing: 'linear',
                    } as SceneAnimation;
            ...
            {/* Pulsating Vignette (Audio Reactive) */}
            {scene.vignette?.enabled && audioPeakEffect > 0.4 && (
                <VignettePulsar 
                    audioPeakEffect={audioPeakEffect} 
                    color={particleColor || '#000'} 
                    intensity={(scene.vignette.intensity || 0.4) * 0.5}
                />
            )}

            {/* Film grain texture - DISABLED to debug crash */}
            {/* ... (existing disabled grain) */}

            {/* Gilded Overlay for authenticity */}
            {scene.id?.includes('gilded') || (scene as any).concept === 'gbs' && (scene as any).genre === 'gilded' ? <GildedOverlay /> : null}

            {/* God-Mode: Audio-reactive Particles */}
            {particleType && (
                <div style={{ position: 'absolute', inset: 0, zIndex: 5 }}>
                    <ParticleBackground 
                         theme={{
                             ...
                             text: { primary: '#fff', secondary: '#fff', accent: '#fff' },
                             caption: { backgroundColor: '', textColor: '', highlightColor: '' },
                             effects: { glowColor: '', shadowIntensity: 0 }
                         }}
                         variant={particleType as any}
                         particleCount={audioPeakEffect > 0.65 ? 120 : 0}
                    />
                </div>
            )}
            {/* Haunted Arrows (Atmospheric Overlay) */}
            {scene.id?.includes('mystery') || (scene as any).genre === 'noir' ? (
                <HauntedArrow 
                    color={particleColor || '#fff'} 
                    glowColor={particleColor || '#f2a900'} 
                    variant="ornate"
                    positions={[
                        { x: 15, y: 30, rotation: 160 },
                        { x: 85, y: 70, rotation: -20 }
                    ]}
                    opacity={0.25}
                />
            ) : null}
