...
- `out/thumbnail-<slug>.png` + `-b.png` + `-c.png` (1280×720, hook already baked in) — rendered by `node scripts/render-thumbnails.js --slug=<slug>` (make-book step 8) from `youtube-meta.json → thumbnail.grammar / thumbnail.variants`. Upload all three in YouTube Studio → Thumbnail → **Test & compare** (the block in `youtube.md` lists them). **Never pass `--gl=angle`** on this GPU-less machine.

### Thumbnail grammar (2026-09-24 — REQUIRED for every new book, both engines)
Never one template (YPP "mass-produced" signal). `node scripts/thumbnail-grammar.js --slug=<slug> --write` (make-book 5.4) picks — deterministic, no LLM — the layout × text side × type × photo grade × palette accent farthest from the last 8 channel thumbnails, plus B/C variants. Antidote books use `scene-still` (a frozen frame of the book's own scene, `src/engines/antidote/thumbHero.ts`; pin one with `thumbnail.grammar.sceneId`) — **no Flux for Antidote** (`--flux-thumb` opts back in). Every card carries a book anchor (title + author). Preview a feed: `node scripts/preview-thumbnail-grammar.js --slugs=a,b --variants`. Do NOT hard-code a layout/colour in a book's meta unless the operator asks; do NOT touch published books (PUBLISHED_BOOKS.md).

### Thumbnail Art Director (God Mode — New Pipeline)
Replaces sharpness-based single-image generation with a 5-concept CTR-thesis pipeline. Runs automatically in `make-book.js` (steps 5a–5d). Use `--skip-art-director` to fall back to legacy single-image mode.

**Pipeline:**
```
Step 5a  thumbnail-art-director.js  → books/<slug>/thumbnail-concepts.json
           5 concepts × {power | soul | scene | conflict | mystery}
           Each concept: hook + visualSubject + Flux prompt (story-bible specific)
           Deterministic — no API call, <1s

Step 5b  (Vox only; Antidote skips it) gen-thumbnail.py --concepts=...  → public/scenes/<slug>/thumbnail-concept-<angle>.png × 5
...
python scripts/gen-thumbnail.py books/<slug>/youtube-meta.json --concepts=books/<slug>/thumbnail-concepts.json --winner-only
node scripts/thumbnail-grammar.js --slug=<slug> --write   # grammar + A/B/C variants
node scripts/render-thumbnails.js --slug=<slug>                   # out/thumbnail-<slug>{,-b,-c}.png
```

**The 5 concept archetypes:**
| Angle | Hook Style | Visual Strategy | Default Layout |
|---|---|---|---|
