...
/** Make a video-level seed from title + author + optional mood */
export function makeVideoSeed(title: string, author: string, mood: string = 'neutral'): number {
    return hashString(`${title}::${author}::${mood}`);
}
