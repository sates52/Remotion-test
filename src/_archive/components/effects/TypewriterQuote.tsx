...
// Simple fast hash for deterministic jitter
                ...
                >
                    ...
                    <div
                        style={{
                            ...
                            opacity: 0.9, // slightly washed ink
                        }}
                    >
                        {displayText.split('').map((char, i) => {
                            // Authentic typewriter jitter
                            const jitterY = (hashString(`jitter-y-${i}`) % 10) / 10 - 0.5;
                            const rot = (hashString(`jitter-rot-${i}`) % 10) / 10 - 0.5;
                            const weightJitter = (hashString(`weight-${i}`) % 2) === 0 ? 0 : 200;
                            return (
                                <span key={i} style={{ 
                                    display: 'inline-block',
                                    transform: `translateY(${jitterY}px) rotate(${rot}deg)`,
                                    fontWeight: 600 + weightJitter,
                                    opacity: 0.85 + ((hashString(`op-${i}`) % 15) / 100),
                                }}>
                                    {char === ' ' ? '\u00A0' : char}
                                </span>
                            );
