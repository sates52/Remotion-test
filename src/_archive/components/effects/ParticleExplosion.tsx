...
export const ParticleExplosion: React.FC<ParticleExplosionProps> = ({
    ...
    // Use triggerFrame to shift when the explosion starts
    const adjustedFrame = Math.max(0, frame - triggerFrame);
    const t = Math.min(adjustedFrame / duration, 1);

    if (adjustedFrame === 0 || adjustedFrame > duration + 10) return null;
